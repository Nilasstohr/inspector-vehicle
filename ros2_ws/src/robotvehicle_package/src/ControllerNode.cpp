...
void ControllerNode::broadCastTF() const {
    ...
    odom_tf.transform.translation.x = missionController->getCurrentPoseX()/100.0;
    odom_tf.transform.translation.y = missionController->getCurrentPoseY()/100.0;
    odom_tf.transform.translation.z = 0.0;

    tf2::Quaternion q;
    q.setRPY(0, 0, missionController->getCurrentPoseTheta());
...
void ControllerNode::publishOdom() const {
    KalmanLocalization* localization = missionController->getLocalization();
    const PredictionDifferentialDrive* prediction = localization->getPrediction();
    ...
    // Position
    odom_msg.pose.pose.position.x = missionController->getCurrentPoseX()/100.0;
    odom_msg.pose.pose.position.y = missionController->getCurrentPoseY()/100.0;
    ...
    const MatrixXd* pEst = prediction->getPEst();
